#-*- coding:utf-8 -*-
"""multik2a.py for pwkvn. Based on similar for gra dictionary
...
def parse_k2s(rec):
 for k2 in rec.k2s:
  hom,k1,status = parse_k2(k2)
  if not status:
   print('parse_k2 error: k2="%s",\nline=%s\n' %(k2,rec.line))
  rec.k1s.append(k1)
 # check that rec.k1 = rec.k1s[0]
 if rec.k1 != rec.k1s[0]:
  print('parse_k2s error: rec.k1=%s, rec.k1s[0]=%s\nline=%s\n' %(rec.k1,rec.k1s[0],rec.line))
